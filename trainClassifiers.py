...
def trainClassifier(X, y, testTweets, testSentiment):
    ...
    searchCV = RandomizedSearchCV(classifier, parametersToTry, n_jobs=-1, error_score=0, n_iter=200, refit=True, cv=3)
    ...
    print 'score on the test data: ' + str( searchCV.score(testTweets, testSentiment) )


    # testPredictions = searchCV.predict_proba(testTweets)
    testPredictions = searchCV.predict(testTweets)
